...
--Default Slot Config Beginning POSIX Time is 1596059091000
--Slot 10 POSIX Time is: 1596059101000
--Slot 10 POSIX Time is: 1596059111000
mkPolicy :: PubKeyHash -> POSIXTime -> () -> ScriptContext -> Bool
mkPolicy pkh deadline () ctx = traceIfFalse "Transaction not signed by owner"  (txSignedBy (scriptContextTxInfo ctx) pkh)      && 
                               traceIfFalse "Deadline has been exceeded" checkDeadLine
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx
        --checks the deadline and returns true if deadline is in range
        checkDeadLine :: Bool
