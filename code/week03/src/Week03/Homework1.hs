...
mkValidator dat () ctx = traceIfFalse "first beneficiary validation failed" checkFirstBeneficiary ||
                         traceIfFalse "second beneficiary validation failed" checkSecondBeneficiary 
    where
        info :: TxInfo
        info = scriptContextTxInfo ctx

        checkFirstBeneficiary :: Bool
        checkFirstBeneficiary = (txSignedBy info $ beneficiary1 dat) && (contains (to $ deadline dat) $ txInfoValidRange info)

        checkSecondBeneficiary :: Bool
        checkSecondBeneficiary = (txSignedBy info $ beneficiary2 dat) && (contains (from $ deadline dat) $ txInfoValidRange info)
